...
async def fetch_bulk_deals():
    ...
    output_filename = f"bulk_deals_{to_date}.json"
    summary_filename = f"bulk_deals_{to_date}_summary.txt"

    logger.info(f"Starting bulk deals download for {from_date} to {to_date}")
    ...
    async with async_playwright() as p:
            ...
            try:
                ...
                if response and response.ok:
                    try:
                        ...
                        with open(f"bulk_deals_raw_response_attempt_{attempt + 1}.txt", "w", encoding='utf-8') as f:
                            f.write(await response.text())
                        logger.info(f"Saved raw response as bulk_deals_raw_response_attempt_{attempt + 1}.txt")
                else:
                ...
                logger.error(f"Attempt {attempt + 1}: Error fetching bulk deals: {e}")
            if attempt < 2:
        ...
        if json_data:
            try:
                ...
                logger.info(f"Original bulk deals JSON saved as {output_filename}")
            except Exception as e:
                logger.error(f"Failed to save original JSON: {e}")

            filtered_data = filter_bulk_deals(json_data)
            if filtered_data:
...
def send_email(summary_filename, date_str):
    """Send email with the bulk deals text summary attached."""
...
async def main():
    filtered_data, summary_filename = await fetch_bulk_deals()
