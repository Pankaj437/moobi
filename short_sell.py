...
def filter_short_sell(data):
    """Filter relevant fields from short selling data."""
    try:
        filtered = []
        for item in data.get('data', []):
            filtered.append({
                'symbol': item.get('SS_SYMBOL', ''),
                'companyName': item.get('SS_NAME', ''),
                'quantity': item.get('SS_QTY', ''),
                'date': item.get('mTIMESTAMP', '')
            })
        logger.info(f"Filtered {len(filtered)} short selling entries.")
        return filtered
    except Exception as e:
        logger.error(f"Failed to filter short selling data: {e}")
        return []

def save_text_summary(data, from_date, to_date, filename):
    """Save filtered short selling data as a human-readable text file."""
    try:
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(f"Short Selling Summary ({from_date} to {to_date})\n")
            f.write("=" * 60 + "\n\n")
            for item in data:
...
async def fetch_short_sell():
    today = datetime.today()
    one_day_ago = today - timedelta(days=1)
    from_date = one_day_ago.strftime("%d-%m-%Y")
    to_date = today.strftime("%d-%m-%Y")
    date_str = today.strftime("%Y-%m-%d")
    output_filename = f"short_sell_{to_date}.json"
    summary_filename = f"short_sell_{to_date}_summary.txt"

    logger.info(f"Starting short selling download for {from_date} to {to_date}")
    ...
    async with async_playwright() as p:
        ...
        api_url = f"https://www.nseindia.com/api/historical/short-selling?from={from_date}&to={to_date}"
        logger.info(f"Fetching short selling data from: {api_url}")
            ...
            try:
                ...
                if response and response.ok:
                    try:
                        ...
                        with open(f"short_sell_raw_response_attempt_{attempt + 1}.txt", "w", encoding='utf-8') as f:
                            f.write(await response.text())
                        logger.info(f"Saved raw response as short_sell_raw_response_attempt_{attempt + 1}.txt")
                else:
                ...
                logger.error(f"Attempt {attempt + 1}: Error fetching short selling data: {e}")
            if attempt < 2:
        ...
        if json_data:
            try:
                ...
                logger.info(f"Original short selling JSON saved as {output_filename}")
            except Exception as e:
                logger.error(f"Failed to save original JSON: {e}")

            filtered_data = filter_short_sell(json_data)
            if filtered_data:
...
def send_email(summary_filename, date_str):
    """Send email with the short selling text summary attached."""
    ...
    msg['From'] = EMAIL_USER
    msg['To'] = EMAIL_TO
    msg['Subject'] = f"Short Selling Data - {date_str}"

    body = f"""Dear Recipient,

Attached is the short selling summary for {date_str} (text format).
...
async def main():
    filtered_data, summary_filename = await fetch_short_sell()
