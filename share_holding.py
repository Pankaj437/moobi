...
def filter_shareholding(data):
    """Filter relevant fields from shareholding data."""
    try:
        filtered = []
        for item in data:
            filtered.append({
                ...
                'shareholdingDate': item.get('shareholdingDate', ''),
                'promoterShare': item.get('promoterShare', ''),
                'institutionalShare': item.get('institutionalShare', ''),
                'publicShare': item.get('publicShare', '')
            })
        logger.info(f"Filtered {len(filtered)} shareholding entries.")
        return filtered
    except Exception as e:
        logger.error(f"Failed to filter shareholding data: {e}")
        return []

def save_text_summary(data, from_date, to_date, filename):
    """Save filtered shareholding data as a human-readable text file."""
    try:
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(f"Shareholding Pattern Summary ({from_date} to {to_date})\n")
            f.write("=" * 60 + "\n\n")
            for item in data:
                f.write(f"Symbol: {item['symbol']}\n")
                f.write(f"Company: {item['companyName']}\n")
                f.write(f"Date: {item['shareholdingDate']}\n")
                f.write(f"Promoter Share: {item['promoterShare']}\n")
                f.write(f"Institutional Share: {item['institutionalShare']}\n")
                f.write(f"Public Share: {item['publicShare']}\n")
                f.write("=" * 60 + "\n\n")
...
async def fetch_shareholding():
    ...
    output_filename = f"shareholding_{to_date}.json"
    summary_filename = f"shareholding_{to_date}_summary.txt"

    logger.info(f"Starting shareholding download for {from_date} to {to_date}")
    ...
    async with async_playwright() as p:
        ...
        api_url = f"https://www.nseindia.com/api/corporates-shareholding?index=equities&from_date={from_date}&to_date={to_date}"
        logger.info(f"Fetching shareholding from: {api_url}")
            ...
            try:
                ...
                if response and response.ok:
                    try:
                        ...
                        with open(f"shareholding_raw_response_attempt_{attempt + 1}.txt", "w", encoding='utf-8') as f:
                            f.write(await response.text())
                        logger.info(f"Saved raw response as shareholding_raw_response_attempt_{attempt + 1}.txt")
                else:
                ...
                logger.error(f"Attempt {attempt + 1}: Error fetching shareholding: {e}")
            if attempt < 2:
        ...
        if json_data:
            try:
                ...
                logger.info(f"Original shareholding JSON saved as {output_filename}")
            except Exception as e:
                logger.error(f"Failed to save original JSON: {e}")

            filtered_data = filter_shareholding(json_data)
            if filtered_data:
...
def send_email(summary_filename, date_str):
    """Send email with the shareholding text summary attached."""
    ...
    msg['From'] = EMAIL_USER
    msg['To'] = EMAIL_TO
    msg['Subject'] = f"Shareholding Pattern Data - {date_str}"

    body = f"""Dear Recipient,

Attached is the shareholding pattern summary for {date_str} (text format).
...
async def main():
    filtered_data, summary_filename = await fetch_shareholding()
